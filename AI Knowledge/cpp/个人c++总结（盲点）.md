...
# 不会的点（带跳转链接）
- [ ] [[C++ 学习手册#^b76695]]   这里这句话表示前面统计整个数组的占用总字节数  后面的sizeof统计数组中单个元素占用的字节数，达到统计整个数组长度的目的。
- [ ]  [[C++ 学习手册#^a7b776]]  c++ 函数中值的传递是副本，你在函数中对参数做修改，不会影响外面参数的值。  如果想用函数直接操纵输入的参数，可以用引用 int& a [[C++ 三小时入门教程#^71d48a]]